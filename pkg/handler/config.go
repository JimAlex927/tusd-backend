...
type Config struct {
	//2025/11/21 这个标志位 是通过concatenate方式合并文件后 决定 是否分文件 会被删除
	// 它如果是真 就一定会删除 如果是假 是否删除 取决于 isFinal的 那个合并请求是否 传递了 deletePart的标志位 todo
	DeletePartAfterConcatenation bool
	//是 本地文件存储 还是 S3 还是其它呢 用这个标志位判断即可
	ComposerCoreType int
	//是否启用 fileStore的 拷贝机制
