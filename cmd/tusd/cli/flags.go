...
import (
	...
	"github.com/JimAlex927/tusd-backend/internal/grouped_flags"
...
var Flags struct {
	...
	//@Amalrich concatenate 合并之后 是否删除子文件
	DeletePartAfterConcatenation bool
	//@Amalrich 是否启动本地文件存储的 拷贝机制
	EnableUploadCopyPath bool
}
...
func ParseFlags() {
	...
	fs.AddGroup("Listening options", func(f *flag.FlagSet) {
		...
		// 合并完成后 文件拷贝的映射路径
		f.StringVar(&Flags.MappingFilePath, "mapping-file-path", "./mapping.json", "mapping for upload-copyPath")
	...
	fs.AddGroup("Upload protocol options", func(f *flag.FlagSet) {
		...
		f.BoolVar(&Flags.DeletePartAfterConcatenation, "delete-partFile-after-concatenation", true, "concatenate合并方式合并文件后子文件是否自动删除")
		f.BoolVar(&Flags.EnableUploadCopyPath, "enable-upload-copy-path", true, "本地文件存储concat 是否进行拷贝映射")
